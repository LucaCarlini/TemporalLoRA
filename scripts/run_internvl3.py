...
from training import (
    ...
    save_model_state,
    save_run_settings,
    set_model_img_context_token_id,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune InternVL with adapters")
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional directory for Hugging Face-format checkpoints (defaults to the run directory).",
    )
    ...
    parser.add_argument(
        ...
        default="none",
        help="Vision-side PEFT choice (encoder QKV).",
    )
    parser.add_argument(
        "--peft_dec",
        type=str,
        ...
        default="lora",
        help="Decoder-side PEFT choice (q_proj/k_proj/v_proj/o_proj).",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="./outputs/internvl3",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional subfolder name inside out_dir.",
    )
    parser.add_argument(
        "--load_pretrained",
        type=str,
        default=None,
        help="Path to a checkpoint (save_pretrained format) to load before training/eval.",
    )
    parser.add_argument(
        "--reload_best",
        action="store_true",
    ...
    parser.add_argument(
        "--zero_shot_eval",
        action="store_true",
        help="Run evaluation only without further fine-tuning.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of frames per EndoVis clip.",
    )
    parser.add_argument(
        "--train_endovis",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of frames to assume when running vision adapters.",
    )
    parser.add_argument(
        "--load_in_8bit",
        action="store_true",
        help="Load base model weights in 8-bit with bitsandbytes.",
    )
    parser.add_argument(
        "--load_in_4bit",
        action="store_true",
        help="Load base model weights in 4-bit with bitsandbytes.",
    )
    parser.add_argument(
        "--max_new_tokens_eval",
        type=int,
        default=64,
        help="Maximum new tokens during evaluation generation.",
    )
    parser.add_argument(
        "--max_samples_eval",
        type=int,
        default=None,
        help="Optional cap on evaluation samples.",
    )
    parser.add_argument(
        "--use_wandb",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Rank for all LoRA-style adapters (LLM, ST-LoRA, TimeShiftLoRA).",
    )
    parser.add_argument(
        "--adapter_alpha",
        type=float,
        default=16.0,
        help="Scaling factor (alpha) for all LoRA-style adapters.",
    )
    parser.add_argument(
        "--adapter_dropout",
        type=float,
        default=0.1,
        help="Dropout for all LoRA-style adapters.",
    )
    parser.add_argument(
        "--st_hidden",
        type=int,
        default=0,
        help="Force hidden size for spatio-temporal adapters (<=0 auto-detect).",
    )
    parser.add_argument(
        "--st_bottleneck",
        type=int,
        default=64,
        help="Bottleneck dimension for ST-LoRA.",
    )
    parser.add_argument(
        "--st_kernel",
        type=int,
        default=3,
        help="Temporal kernel size for ST modules.",
    )
    parser.add_argument(
        "--tslora_hidden",
        type=int,
        default=0,
        help="Force hidden size for TimeShift LoRA (<=0 auto-detect).",
    )
    parser.add_argument(
        "--tslora_n_div",
        type=int,
        default=3,
        help="Number of divided subspaces for shifting.",
    )
    parser.add_argument(
        "--debug_shapes",
        action="store_true",
...
def build_default_run_name(args) -> str:
    ...
    parts.append(f"enc{args.peft_enc}")
    parts.append(f"dec{args.peft_dec}")
...
def run_once(args) -> None:
    ...
    for tok in ("<img>", "</img>", "<IMG_CONTEXT>"):
    ...
    model_file = getattr(sys.modules.get(model_module), "__file__", "unknown")
    print(f"[Model] class={model.__class__.__name__} module={model_module}")
    print(f"[Model] module file={model_file}")
    model.enable_input_require_grads()

    img_context_token_id = tokenizer.convert_tokens_to_ids("<IMG_CONTEXT>")
    set_model_img_context_token_id(model, img_context_token_id)
    ...
    if args.train_endovis:
        train_dataset = build_endovis_train_dataset(args, DEFAULT_ENDOVIS_TRAIN_SEQS)
        valid_dataset = build_endovis_train_dataset(args, DEFAULT_ENDOVIS_VAL_SEQS)
    else:
        train_dataset = build_real_dataset(args, args.real_annotations_path, mode="train")
        try:
            valid_dataset = build_real_dataset(args, args.real_annotations_path, mode="val")
    ...
    if clip_length is None:
        if args.train_endovis:
            ...
            clip_length = infer_clip_length_from_dataset(train_dataset)
    ...
    frames_limit = args.frames if getattr(args, "frames", 0) and args.frames > 0 else None
    ...
    num_image_token = getattr(model, "num_image_token", 576)
    ...
    eval_collator = VideoQAEvalCollator(max_frames=frames_limit)
    test_dataset = None
    ...
    if args.eval_real:
        test_dataset = build_real_dataset(args, args.real_annotations_path, mode="test")
    ...
    if args.eval_real_ood:
        if Path(args.real_ood_annotations_path).exists():
            real_ood_dataset = build_real_dataset(args, args.real_ood_annotations_path, mode="test")
    ...
    if args.use_wandb:
        ...
        wandb_module = wandb
        wandb_kwargs = dict(
            project=args.wandb_project or "train_internvl3",
    ...
    if args.epochs <= 0:
        print("[Mode] No training epochs requested; running evaluations only.")
        do_evaluations(
            model,
            processor,
            device,
...
def main():
    ...
    args.out_dir = str(output_root / "outputs" / f"internvl3_{dataset_tag}_{args.epochs}{out_suffix}")
